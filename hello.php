<?php include("assets/inc/header.inc.php"); ?>

				<!-- Main -->
					<div id="main" class="alt">

						<!-- One -->
							<section id="one">
								<div class="inner">
									<div class="row">
										<div class="col-lg-8">
											<header class="major">
												<h1>Hello-World</h1>

												<h4>All Languages</h4>

											</header>
										</div>

										<div class="col-lg-4 text-right">

											<br>
											<br>

											<ul class="icons">
												<!--<li><a href="https://www.facebook.com/guillaume.dupuy.56/" class="icon alt fa-facebook"><span class="label">Facebook</span></a></li>-->
												<!--<li><a href="https://www.instagram.com/guill_dup/" class="icon alt fa-instagram"><span class="label">Instagram</span></a></li>-->
												<li><a href="https://www.linkedin.com/in/guillaume-dupuy/" class="icon alt fa-linkedin"><span class="label">LinkedIn</span></a></li>
											</ul>

											<a href="contact" class="button next scrolly">Contact</a>
										</div>
									</div>
									
									<br>
								</div>
							</section>

							<section id="contact">
								<div class="inner">
									<section>
										<header class="major">
											<h2>Description</h2>
										</header>

							            <p> Le but du projet est d'écrire des scripts permettant de faire ""Hello World"" dans tout les languages de programmation existants.
							              </p>    

										<button><a href="https://github.com/tot0p/Hello-World">Accès Repo Github</a></button>

									</section>

									<section class="split">
										<section>
											<img src="images/hello.png" class="img-responsive2" alt="">
										</section>
									</section>

								</div>
							</section>
					</div>

<?php include("assets/inc/footer.inc.php"); ?>